...
import { redistClient } from "../index.js";
...
export const registerUser = TryCatch(async (req, res, next) => {
  ...
  if (role === "recruiter") {
    ...
    if (!file) {
      throw new ErrorHandler(400, "Resume file required for jobseekers");
    }
...
export const loginUser = TryCatch(async (req, res, next) => {
  ...
  res.status(200).json({
    message: `Welcome Back ${userObject.name}`,
    user: userObject,
    token,
  });
});
...
export const forgotPassword = TryCatch(async (req, res, next) => {
  ...
  if (!email) {
    throw new ErrorHandler(400, "Please enter your email");
  }
  ...
  await redistClient.set(`forgot:${email}`, resetToken, {
    EX: 900,
  });
...
export const resetPassword = TryCatch(async (req, res, next) => {
  ...
  try {
    ...
    throw new ErrorHandler(400, "Token Expired or Invalid");
  }
  ...
  const storedToken = await redistClient.get(`forgot:${email}`);
  ...
  await redistClient.del(`forgot:${email}`);
